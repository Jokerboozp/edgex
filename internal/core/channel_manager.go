...
type ChannelManager struct {
	...
	deviceIDs             map[string]struct{} // 全局设备 ID 集合，O(1) 唯一性检查
}
...
func (cm *ChannelManager) AddChannel(ch *model.Channel) error {
	...
	// 设备 ID 前置规范化：EnsureDeviceID 保证每个设备有非空 ID
	// 必须在 sanitizeDeviceConfig / autoGenerateDLT645PointsFromConfig 之前，
	// 因为后者调用 GenerateDLT645StandardPoints 依赖 dev.ID。
	// Pre-normalize device IDs BEFORE any config/point generation —
	// autoGenerateDLT645PointsFromConfig calls GenerateDLT645StandardPoints(dev.ID).
	...
	// 格式化所有设备配置 + 自动点位生成（此时 dev.ID 已保证非空）
	for i := range ch.Devices {
	...
	// 通道内去重 + 全局唯一性校验 — 在 driver Init 之前 Fail-Fast
	// Intra-channel dedupe + global uniqueness — fail-fast before driver Init.
	seenDeviceIDs := make(map[string]struct{}, len(ch.Devices))
	for i := range ch.Devices {
		if _, dup := seenDeviceIDs[ch.Devices[i].ID]; dup {
			return fmt.Errorf("duplicate device %s in channel %s", ch.Devices[i].ID, ch.ID)
		}
		if cm.deviceIDExistsGloballyLocked(ch.Devices[i].ID) {
			return fmt.Errorf("device %s already exists globally", ch.Devices[i].ID)
		}
		seenDeviceIDs[ch.Devices[i].ID] = struct{}{}
	}

	// 初始化驱动（所有参数校验已通过）
	// Init driver — all parameter checks passed above.
	...
	for i := range ch.Devices {
		cm.deviceIDs[ch.Devices[i].ID] = struct{}{}
...
func (cm *ChannelManager) UpdateChannel(ch *model.Channel) error {
	// 1. Stop existing channel（停采集，不删状态 — StopChannel 只做 ScanEngine 注销 + Disconnect）
	...
	// 计算 delta：旧设备集合 vs 新设备集合，供后续"删旧加新"使用。
	// Compute delta between old and new device sets for cleanup below.
	existing, hasExisting := cm.channels[ch.ID]
	oldDeviceIDs := make(map[string]struct{})
	if hasExisting && existing != nil {
		for _, d := range existing.Devices {
			oldDeviceIDs[d.ID] = struct{}{}
		}
	}

	// 设备 ID 规范化 + 通道内去重 + 全局唯一性校验
	// UpdateChannel 场景：StopChannel 不清理 cm.deviceIDs，需在检查前先把旧设备 ID 临时移除，
	// 否则自身设备 ID 会被"豁免"但状态脏（重命名 A→B 后 A 残留）。
	// 更干净的做法：用 oldDeviceIDs 计算"新集合"与"旧集合"的差集，先临时从 deviceIDs 删掉将被移除的旧 ID，
	// 让全局检查在干净的 deviceIDs 上进行；校验通过后再统一写回。
	//
	// 简化方案：校验时不豁免 self，而是在临时"剥离"将被替换的旧设备 ID 之后再做全局检查。
	// Pre-check strategy: first purge deviceIDs of devices that are NOT in the new list,
	// then run the global uniqueness check on a clean state.

	newDeviceIDs := make(map[string]struct{}, len(ch.Devices))
	for i := range ch.Devices {
		if err := model.EnsureDeviceID(&ch.Devices[i]); err != nil {
			return err
		}
		if _, dup := newDeviceIDs[ch.Devices[i].ID]; dup {
			return fmt.Errorf("duplicate device %s in channel %s", ch.Devices[i].ID, ch.ID)
		}
		newDeviceIDs[ch.Devices[i].ID] = struct{}{}
	}

	// 临时剥离本通道所有旧设备 ID（StopChannel 不清理 deviceIDs），
	// 让后续全局唯一性检查在干净的全局视图下运行 —— 保留自身的设备 ID 也不再豁免，
	// 因为暂时从 deviceIDs 中移除后它们就和"新增"设备地位相同了。
	// Strip ALL old device IDs from deviceIDs so the global uniqueness check
	// runs on a clean global view (no self-exemption needed). After validation
	// passes we write the new device set back atomically.
	for oldID := range oldDeviceIDs {
		delete(cm.deviceIDs, oldID)
	}

	// 全局唯一性校验（此时 deviceIDs 中完全没有本通道的旧设备 → 新列表中任何 ID
	// 只要和其他通道冲突就会被拦截；保留自身的设备也不再 self-collide）。
	// Global check — self-collision impossible since we stripped all old IDs above.
	for newID := range newDeviceIDs {
		if cm.deviceIDExistsGloballyLocked(newID) {
			// 校验失败 → 事务回滚：恢复所有刚剥离的旧 ID
			for oldID := range oldDeviceIDs {
				cm.deviceIDs[oldID] = struct{}{}
			}
			return fmt.Errorf("device %s already exists globally", newID)
		}
	}

	// 计算 delta：找出"旧有但新列表没有"的设备 ID（这些需要清理 shadow/stateManager/tagRegistry）
	// Compute stale IDs for shadow/stateManager cleanup.
	staleIDs := make(map[string]struct{})
	for oldID := range oldDeviceIDs {
		if _, stillPresent := newDeviceIDs[oldID]; !stillPresent {
			staleIDs[oldID] = struct{}{}
		}
	}

	// 清理 stale 设备的状态
	for staleID := range staleIDs {
		cm.stateManager.UnregisterNode(staleID)
		cm.tagRegistry.UnregisterDevice(ch.ID, staleID)
		if sc := cm.shadowCore; sc != nil {
			shadowID := fmt.Sprintf("shadow-%s", staleID)
			if err := sc.DeleteShadowDevice(shadowID); err != nil {
				zap.L().Debug("Shadow cleanup skipped during update",
					zap.String("shadow", shadowID), zap.Error(err))
			}
		}
	}

	// 写回新 deviceIDs（所有旧 ID 已在上面被剥离，这里直接写回新集合）
	// Write back new deviceIDs — all old IDs were stripped above.
	for newID := range newDeviceIDs {
		cm.deviceIDs[newID] = struct{}{}
	}
...
func (cm *ChannelManager) RemoveChannel(channelID string) error {
	...
	// Cascade cleanup: stateManager, shadow, tagRegistry. ShadowCore key is "shadow-<deviceID>".
	cm.stateManager.UnregisterNode(channelID)
	for _, dev := range ch.Devices {
		cm.stateManager.UnregisterNode(dev.ID)
		delete(cm.deviceIDs, dev.ID)
		if sc := cm.shadowCore; sc != nil {
			shadowID := fmt.Sprintf("shadow-%s", dev.ID)
			if err := sc.DeleteShadowDevice(shadowID); err != nil {
				zap.L().Debug("Shadow cleanup skipped (may never have existed)",
					zap.String("shadow", shadowID), zap.Error(err))
			}
		}
...
func (cm *ChannelManager) appendDeviceLocked(ch *model.Channel, channelID string, dev *model.Device) error {
	...
	// 设备 ID 为全局主键：同一 ID 不允许在全系统任何通道/协议中出现第二次。
	// Device ID is a global primary key — it must be unique across all channels and protocols.
	if cm.deviceIDExistsGloballyLocked(dev.ID) {
		return fmt.Errorf("device %s already exists", dev.ID)
	}

	bacnetNewID, bacnetHasNewID := getDeviceID(dev.Config)
	for _, d := range ch.Devices {
		if ch.Protocol == "bacnet-ip" {
			if bacnetHasNewID {
				if oldID, okOld := getDeviceID(d.Config); okOld && bacnetNewID == oldID {
					return fmt.Errorf("BACnet device with Instance ID %d already exists", bacnetNewID)
				}
			}
		}
	}
...
// deviceIDExistsGloballyLocked reports whether a device ID already exists in ANY
// channel (all protocols). It MUST be called with cm.mu held.
// 设备 ID 全局唯一性检查：在任意通道/协议中已存在即返回 true。调用方需持有 cm.mu。
// deviceIDExistsGloballyLocked reports whether id is used by any device in any channel.
// O(1) via the global deviceIDs index. Caller must hold cm.mu.
func (cm *ChannelManager) deviceIDExistsGloballyLocked(id string) bool {
	_, exists := cm.deviceIDs[id]
	return exists
}
...
func (cm *ChannelManager) BatchAddModbusSlaves(channelID string, slaveStart, slaveEnd, regStart, regEnd int, interval model.Duration, enable bool, datatype, readwrite string, regType model.RegisterType, fc byte) (*BatchAddModbusSlavesResult, error) {
	...
	// 设备 ID 为全局主键，用通道前缀保证跨通道生成的从站 ID 不冲突
	// Device ID is a global primary key; prefix it with the channel to stay unique across channels.
	idPrefix := fmt.Sprintf("%s-modbus-slave", channelID)
...
func (cm *ChannelManager) UpdateDevice(channelID string, dev *model.Device) error {
	...
	// 编辑时同样校验 BACnet 实例号唯一性（与 appendDeviceLocked 新增逻辑一致），
	// 防止把某台的 bacnet_device_id/IP 改成与另一台相同，导致驱动把两台当作同一实例扫描，
	// 进而在点位层把两台设备合并成一台。编辑本设备(跳过 idx)后，其它设备不得占用相同实例号。
	// Editing must not create a BACnet instance conflict, otherwise the driver treats two devices
	// as one object instance and their points get merged into a single device.
	if ch.Protocol == "bacnet-ip" {
		if newID, okNew := getDeviceID(dev.Config); okNew {
			for i, d := range ch.Devices {
				if i == idx {
					continue
				}
				if oldID, okOld := getDeviceID(d.Config); okOld && newID == oldID {
					return fmt.Errorf("BACnet Device ID %d is already in use by device %s", newID, d.ID)
				}
			}
		}
	}
...
func (cm *ChannelManager) RemoveDevice(channelID, deviceID string) error {
	...
	// 级联清理影子设备（ShadowCore key = "shadow-<deviceID>"）
	// Cascade shadow cleanup — ShadowCore key is prefixed with "shadow-".
	if sc := cm.shadowCore; sc != nil {
		shadowID := fmt.Sprintf("shadow-%s", oldDev.ID)
		if err := sc.DeleteShadowDevice(shadowID); err != nil {
			zap.L().Debug("Shadow cleanup skipped (may never have existed)",
				zap.String("shadow", shadowID), zap.Error(err))
		}
...
func (cm *ChannelManager) RemoveDevices(channelID string, deviceIDs []string) error {
	...
	for _, d := range ch.Devices {
		if toRemove[d.ID] {
			...
			delete(cm.deviceIDs, d.ID)
			// 级联清理影子设备
			if sc := cm.shadowCore; sc != nil {
				shadowID := fmt.Sprintf("shadow-%s", d.ID)
				if err := sc.DeleteShadowDevice(shadowID); err != nil {
					zap.L().Debug("Shadow cleanup skipped (may never have existed)",
						zap.String("shadow", shadowID), zap.Error(err))
				}
